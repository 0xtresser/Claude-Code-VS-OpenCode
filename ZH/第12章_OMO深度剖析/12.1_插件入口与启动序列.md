...
```mermaid
sequenceDiagram
    participant OC as OpenCode 宿主
    participant OMO as OMO 插件
    participant Reg as OpenCode 注册表
    
    OC->>OMO: 加载插件 (src/index.ts)
    activate OMO
    OMO->>OMO: 阶段 1：初始化 providers
    OMO->>OMO: 阶段 2：构建插件组件
    OMO->>OMO: 阶段 3：创建 agents（11）
    OMO->>OMO: 阶段 4：创建 tools（26）
    OMO->>OMO: 阶段 5：设置 MCP（3 个内建）
    OMO->>OMO: 阶段 6：加载 commands
    OMO->>Reg: 注册 8 个 hook handler
    OMO-->>OC: 插件就绪
    deactivate OMO
    
    Note over OC,OMO: 第一条用户消息到达
