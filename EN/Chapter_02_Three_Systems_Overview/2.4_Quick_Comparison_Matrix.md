...
| **Model support** | 20+ providers via Vercel AI SDK in `/0-clones/opencode/packages/opencode/src/provider/provider.ts` | Multi-model role routing via semantic categories in `/0-clones/oh-my-opencode/src/config/schema/categories.ts` | Anthropic first-party plus Bedrock, Vertex, Foundry-style provider chain in `/25-Claude-Code/src/utils/model/` |
...
| **Tool count** | ~22 built-in tools under `/opencode/packages/opencode/src/tool/` | 26+ custom tools layered atop OpenCode tools | 61 built-in tool directories under `/25-Claude-Code/src/tools/` |
